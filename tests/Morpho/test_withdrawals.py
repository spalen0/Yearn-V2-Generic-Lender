...
def test_simple_withdraw(
    ...
    plugin = interface.IGeneric(status[0][3])
    balance_before = plugin.nav()
    plugin.withdraw(depositAmount, {"from": strategist})
    assert plugin.nav() == balance_before - depositAmount
