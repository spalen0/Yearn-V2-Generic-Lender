...
def test_clone(
    ...
    original_comp = GenericAaveMorpho.at(strategy.lenders(strategy.numLenders() - 1))
    cloned_name = "Cloned_Comp_" + currency.symbol()
    tx = original_comp.cloneMorphoAaveLender(
        cloned_strategy, cloned_name, pool_token, {"from": gov}
    )
    cloned_lender = GenericAaveMorpho.at(tx.return_value)
    assert cloned_lender.lenderName() == cloned_name

    cloned_strategy.addLender(cloned_lender, {"from": gov})

    with brownie.reverts():
        cloned_lender.initialize(pool_token, {"from": gov})
